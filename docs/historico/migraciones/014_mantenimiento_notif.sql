-- 014_mantenimiento_notif.sql — el reaper devuelve notificaciones listas para
-- que wf_cron las despache por wf_enviar (chat_id + plantilla + vars).
...
BEGIN
    ...
    SELECT coalesce(jsonb_agg(jsonb_build_object(
             ...
             'respuestas', jsonb_build_array(jsonb_build_object(
                 'plantilla','ejecucion.fallida','vars','{}')))), '[]')
      INTO v_notif
    ...
    SELECT v_notif || coalesce(jsonb_agg(jsonb_build_object(
             'chat_id', u.telegram_chat_id,
             'respuestas', jsonb_build_array(jsonb_build_object(
                 'plantilla','sesion.recordatorio','vars','{}')))), '[]')
      INTO v_notif
